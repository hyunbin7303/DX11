...
void DX::RenderFrame(void)
{
	...
	for (int i = 0; i < 10; i++)
	{
		m_model->Rendering(devcon, ManyCubes_world[i]);
	}
...
void DX::Update(float time)
{
	...
	angle += 90.0f * time;
	XMVECTOR rotateAxis = XMVectorSet(0, 1, 0, 0);
	Cube1_world = XMMatrixRotationAxis(rotateAxis, XMConvertToRadians(angle));

	// Generate 10 cubes behind the origin cube.
	for (int i = 0; i < 10; i++)
	{
		float location = i * 3.5f - 15.0f; // setting up location
		XMMATRIX mTranslate = XMMatrixTranslation(location, 4.0f, 20.0f);
		ManyCubes_world[i] = mTranslate;  //Move the location by using translation.
	}
...
void DX::ClearScreen()
{
	...
	devcon->ClearDepthStencilView(g_DepthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);
